...
def execute_sql(task_level):
    ...
    where job_level='{task_level}' and job_type is null
    order by level_sort'''

    df = pd.read_sql(sql, gp_con)
    deal_infos = []
    ...
    for index, (job_db, job_name, job_sql, job_level, job_owner) in df.iterrows():
        ...
        deal_infos.append([job_name, job_sql, begin_time, end_time, job_result, job_db, job_level, job_owner])
    columns = ['job_name', 'job_sql', 'begin_time', 'end_time', 'job_result', 'job_db', 'job_level', 'job_owner']
...
scheduler.add_job(execute_sql, 'cron', hour='2', minute='0', second='0', args=['dwd'],
                  jitter=random.randint(0, 60), misfire_grace_time=600)
scheduler.add_job(execute_sql, 'cron', hour='3', minute='30', second='0', args=['dws'],
                  jitter=random.randint(0, 60), misfire_grace_time=600)
scheduler.add_job(execute_sql, 'cron', hour='4', minute='0', second='0', args=['ads'],
                  jitter=random.randint(0, 60), misfire_grace_time=600)

# scheduler.add_job(execute_sql, 'interval', seconds=10, args=['ads'],
