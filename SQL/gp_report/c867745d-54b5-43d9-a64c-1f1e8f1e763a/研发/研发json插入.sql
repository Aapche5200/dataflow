...
delete
from ods_gp_job_schedule_pool
where job_name in (select job_sql
                   from ods_gp_job_schedule_pool
                   where job_owner = '尹书山'
                     and job_name = concat('func_', 'ads_release_table_qty')
                   limit 1)

update ods_task_job_schedule_pool
set job_sql ='update formmain_129544
...
where job_name='func_formmain_129544'

update ods_gp_job_schedule_pool
set job_sql='truncate table dwd_paas_uf_product_infov2;
insert into dwd_paas_uf_product_infov2
select id,                     --id
       requestid,              --流程ID
       formmodeid,             --paas内模块ID
       modedatacreater,        --创建人id 跟OA表ID 关联
       modedatacreatertype,    --创建人类型
       modedatacreatedate,     --创建日期
       modedatacreatetime, --创建时间
       modeuuid,               --系统内数据uuid，不用管
       form_biz_id,            --表单标识id，不用管
       xszx,                   --销售属性
       cpxt,                   --产品形态
       cpxbm,                  --产品线编码
       cpfl,                   --产品分类
       ...
       transfer_date,          --同步日期
       transfer_time           --同步时间
from ods_paas_uf_product_infov2
;' --,level_sort=151
where job_name = concat('func_','dwd_pass_uf_product_infov2')
...
job_desc,
job_owner)
values ('func_dwd_paas_uf_productmodel_infov2',
       'dwd',
       111,
...
select id,                           --id
       ...
       tboaptbj,                     --同步oa平台标记
       tbxkptbj,                     --同步许可平台标记
       sfts,                         --是否退税，废弃功能字段不再使用
       cptsmcqd,                     --产品退税名称清单，废弃功能字段不再使用
       transfer_date,                --同步日期
       transfer_time                 --同步时间
