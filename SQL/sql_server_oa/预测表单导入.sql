...
insert into formmain_129544
(field0001, field0002, field0003, field0004, field0006, field0007, field0010, field0008, field0012, field0005,
 field0014, ID)
    (select t1.*,
     ...
     from (select confirm_date as field0001,--确认日期
           ...
           from (SELECT (GETDATE() - 1)                                  dateid,    --当前存入分区
                 ...
                 from (SELECT field0004                                                                           料号,
                              ...
                              '2023'                                                                              year_date,
                              'Q1'                                                                                季度,
                              CONVERT(char(10), '2023-03-31')                                                     确认收入日期,
                              field0291                                                                           待确认收入类型,
                              field0002                                                                           合同,
                              sum(field0278)                                                                      确认收入,
...
insert into formmain_129544 (field0001,field0002,field0003,field0006,ID)
(
select '2023-06-30'                field0001,
       field0002,
       field0003,
       field0006,
       (row_number() over (order by field0003)) + 1000000000000100 as ID
from (select cast(area as varchar(800))                      field0002,
             cast(area_unit as varchar(800))                 field0003,
             round(sum(coalesce(income_diff, 0)) / 10000, 0) field0006
      from ads_gp_oa_t_area_unit_sales_dd
      where cast(area as varchar(800)) in ('其他',
                                           '其它（非营销中心）',
                                           '其它（营销中心）')
      group by cast(area as varchar(800)), cast(area_unit as varchar(800))
      ) as t
)