...
having  count(*)>=2
--①产线员工数量
-- delete  from ads_gp_oa_t_hr_eminf_inf_df where date(dateid)=date('2023-04-16')
--select *  from ads_gp_oa_t_hr_eminf_inf_df where date(dateid)=date('2023-04-16')
insert into ads_gp_oa_t_hr_eminf_inf_df
select tt1.cpxname                            as depart_cpx,
       count(distinct field0002)              as yuangong_num,
       (getdate() - 1) as dateid,
       null                                   as none_two
from (select case when t2.一级部门 = '智能检测与终端产品线' then '智能检测与终端产品线' else t2.一级部门 end as cpxname,
             t1.field0002,
             t1.field0006                                                                                em_name,
             t2.一级部门
      from formmain_10037 as t1
               join depart_level_all as t2 on t1.field0200 = t2."部门ID"
      where field0180 = '-7808949530931608431'
        and t2.一级部门 in ('智能检测与终端产品线', '基础安全产品线', 'AiLPHA大数据智能安全产品线', '云产品线')
      group by case when t2.一级部门 = '智能检测与终端产品线' then '智能检测与终端产品线' else t2.一级部门 end, t1.field0002,
               t1.field0006, t2.一级部门) as tt1
...
from (select convert(varchar(100),a.field0028,120)                                                                   日期,
             --field0010                                                                           工号,
             a.field0091                                                                         一级行业,
             a.field0092                                                                         二级行业,
             a.field0275                                                                         三级行业,
      ...
      from formmain_105321 as a
               left join ods_oa_finance_cp_map_relation as b on a.field0004 = b.item_code
      where convert(varchar(100),field0028,120) >= convert(varchar(100),'2022-01-01',120)
        and convert(varchar(100),field0028,120) < convert(varchar(100),getdate(),120)
      GROUP BY convert(varchar(100),a.field0028,120) ,
               --field0010,
               field0091,
               field0092,
...
from (select date(t1.field0028)                                                                  日期,
      ...
      from formmain_105321 as t1
               left join org_unit as t2 on t1.field0183 = cast(t2."ID" as text)
...
from (select date(t1.field0018)           日期,
      ...
      from formmain_2156 t1
               left join formson_2157 t2 on t1."ID" = t2.formmain_id
               left join ctp_enum_item t3 on cast(t1.field0160 as text) = cast(t3."ID" as text)
               left join ctp_enum_item t4 on cast(t1.field0161 as text) = cast(t4."ID" as text)
               left join ctp_enum_item t5 on cast(t1.field0282 as text) = cast(t5."ID" as text)
...
from (select date(t1.field0018)           日期,
      ...
      from formmain_2156 t1
               left join formson_2157 t2 on t1."ID" = t2.formmain_id
               left join org_unit as t3 on t1.field0040 = cast(t3."ID" as text)
      ...
      from ex_ods_pass_ecology_matrixtable_6
            ...
            (SELECT a.id        员工系统自带ID,
             ...
             FROM ex_ods_pass_ecology_hrmresource as a
                      left join formmain_10037 as b on a.loginid = b.field0002
             GROUP BY a.id,
...
--    array_to_string(ARRAY_AGG(a.id), ', ') as 反馈项目总数ID列表,--增加工单ID，用于产品下钻
       ...
       count(distinct case
                          ...
                          else null end)                                       as 遗留未解决项目数,
--    array_to_string(ARRAY_AGG(case when a.work_order_status not in (0, 5) then a.id  else null  end), ', ') as 遗留未解决项目ID列表, --增加工单ID，用于产品下钻
       ROUND((count(case when a.work_order_status not in (0, 5) then 1 else null end) /
              count(distinct a.id) :: NUMERIC), 2)                             as 闭环率
from ex_ods_itr_workorder_work_order a
where a.work_order_status <> 0
...
insert into ads_gp_pg_t_itr_user_hangye_df
    (with formson_table as (select "ID", formmain_id
     ...
     from ex_ods_itr_workorder_work_order a
              ...
              inner join formmain_1119 c on b.formmain_id = c."ID"
     where a.work_order_status <> 0
...
from ex_ods_itr_workorder_work_order a
         ...
         FULL OUTER JOIN (select "ID", formmain_id
                          ...
                          union all
                          select "ID", formmain_id
                          from formson_19389) e on a.product_line_id = cast(e."ID" as varchar(64))
         FULL OUTER JOIN (select "ID", field0002 as hetong_id, field0004 as kehu_name, field0976 as shangji_name
                          ...
                          from formmain_18955) f on e.formmain_id = f."ID"
         FULL OUTER JOIN formson_1120 g on g.formmain_id = f."ID"
where a.work_order_status <> 0;
...
select a.field0182                                   cpid,--     as 产品ID,
       ...
       null as                                       none_two,
       null as                                       none_three
from formmain_1294 a
         inner join formson_1939 b on a.field0008 = b.field0174
         inner join formmain_1119 c on b.formmain_id = c."ID"
...
--12-2023 经营结果数据表
--delete  from ads_gp_all_t_cpx_cp_zhibiao_df where date(dateid)=date('2023-04-25');
--select * from ads_gp_all_t_cpx_cp_zhibiao_df where date(dateid)=date('2023-04-25');
...
select 产品模块料号,产品模块名称,产品线编码,产品线,产品分类编码,产品分类,产品编码,产品名称
    from(
SELECT
    t1.产品型号编码,
    t1.产品型号名称,
    t1.产品模块料号,
    t1.产品模块名称,
    t2.产品线编码, t2.产品线, t2.产品分类编码, t2.产品分类, t2.产品编码, t2.产品名称
    from(
select  field0001 产品型号编码,
field0002 产品型号名称,
field0003 产品模块料号,
field0004 产品模块名称
FROM
...
field0009,
field0010
) as t2 on t1.产品型号编码=t2.产品型号编码
) as t
group by 产品模块料号,产品模块名称,产品线编码,产品线,产品分类编码,产品分类,产品编码,产品名称