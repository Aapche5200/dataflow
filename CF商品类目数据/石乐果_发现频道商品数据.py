...
start_input = '2020-02-27'
end_input = '2020-03-04'
...
e.item_no as 货号, h.front_cate_one as 一级类目, h.front_cate_two as 二级类目, h.front_cate_three as 三级类目,h.write_uid,
h.product_level as 商品等级,b.pv_impression as 曝光PV,b.uv_impression as 曝光UV,c.pv_click as 点击PV,c.uv_click as 点击UV,
d.uv_add 加购UV,a.uv_pay as 支付UV, a.so_num as 订单数,a.gmv as GMV,
...
left join supply_chain.supply_chain_risk_score_info_log as g on g.item_id=h.item_no
ORDER BY 1,2,3,4
...
if __name__ == '__main__':
    ...
    # 写入excel 操作
    writer = pd.ExcelWriter('发现频道商品数据' + time.strftime('%Y-%m-%d', time.localtime(time.time())) + '.xlsx')
