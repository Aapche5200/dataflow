...
where a.log_date BETWEEN getdate()-9 and getdate()-1 and a.front_cate_one='全类目' and a.yewu_type='total'
ORDER BY a.log_date,a.front_cate_one,a.yewu_type
''')

sql_ms_return = ('''
SELECT week, old_cate_one,yewu_type,tuihuolv
from CFcategory.dbo.CategoryReturn
where week BETWEEN DATENAME(week, GETDATE())-8 and DATENAME(week, GETDATE())-5 and old_cate_one='全类目' and yewu_type='total'
ORDER BY week DESC
''')

sql_ms_dau = ('''
SELECT event_date,dau from CFflows.dbo.CfDau
where event_date BETWEEN getdate()-9 and getdate()-1 
''')
