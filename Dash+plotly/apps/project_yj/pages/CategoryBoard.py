...
WHERE (二级类目 = ' ' or 二级类目 is null) and ( 三级类目=' ' or 三级类目 is null)  and 日期 is not null and 日期 BETWEEN date_sub(curdate(),interval 360 day) and date_sub(curdate(),interval 1 day)
ORDER BY 日期
''')

sql_product_level = (
    '''SELECT a.log_date,a.product_level,a.front_cate_one,b.日期 as Quarter_Four,a.item_num,
b.商品数量 as mubiao_num,a.item_num/b.商品数量 as done_lv
from  CFcategory.商品等级数据_前台 as a
join CFcategory.商品等级目标表 as b on a.product_level=b.商品等级 and a.front_cate_one=b.前台一级类目
where a.log_date BETWEEN date_sub(curdate(),interval 90 day) and date_sub(curdate(),interval 1 day)
''')

sql_level_change = ('''
select * from CFcategory.product_level_change_7days
where log_date BETWEEN date_sub(curdate(),interval 90 day) and date_sub(curdate(),interval 1 day)
ORDER BY log_date
''')

# 业务线数据-做占比图
sql_yewuxian = (
    '''SELECT 日期 as log_date,业务类型 as yewuxian,一级类目 as front_cate_one,sum(支付金额) as gmv ,SUM(独立访客) as uv
from CFcategory.category_123_day
WHERE 一级类目 ='全类目' and (二级类目 = ' ' or 二级类目 is null) and ( 三级类目=' ' or 三级类目 is null)
and 日期 BETWEEN date_sub(curdate(),interval 90 day) and date_sub(curdate(),interval 1 day)
...
from CFcategory.category_123_day
WHERE 业务类型='total'  and (三级类目 = ' ' or 三级类目 is null) and (二级类目 = ' ' or 二级类目 is null) and 日期  BETWEEN date_sub(curdate(),interval 90 day) and date_sub(curdate(),interval 1 day)
GROUP BY 日期 ,一级类目
ORDER BY 日期 ,一级类目
''')
...
@app.callback([Output('my-graph-kpi-toubu',
                      'figure'),
               Output('my-graph-kpi-yaobu',
                      'figure'),
               Output('my-graph-yewuxian',
                      'figure'),
               Output('my-graph-catezhanbi',
                      'figure')],
              [Input('date-picker-range-level-kpi',
                     'start_date'),
               Input('date-picker-range-level-kpi',
                     'end_date')],
              )
def update_level_kpi_toubu(start_date, end_date):
    # 画图进图条图 数据处理
    data_product_level.log_date = pd.to_datetime(
        data_product_level.log_date, format='%Y-%m-%d')
    filter_data_product_level_total = data_product_level[(
        data_product_level.log_date >= start_date) & (data_product_level.log_date <= end_date)]
    ...
    data_product_level_total.rename(
        columns={
            'item_num': 'item_num_mean',
            'mubiao_num': 'mubiao_num_mean',
            'done_lv': 'done_lv_mean',
            'mubiao_lv': 'mubiao_lv_mean'},
        inplace=True)
    print("头部腰部汇总数据-平均值")
    # 排序 使得做出来的图降序排序
    ...
    fig_kpi_toubu = go.Figure(go.Bar(y=filter_data_product_level_total_toubu.front_cate_one,  # 横向条形图Y为类目值
                                     ...
                                     marker=dict(
                                         ...
                                         line=dict(
                                             color='rgb(0, 81, 108)', width=1)  # 设置条形图边框颜色及宽度
                                     ),  # text是在图形内设置标签
    ...
    fig_kpi_toubu.add_trace(
        go.Bar(
            y=filter_data_product_level_total_toubu.front_cate_one,
    ...
    annotations_done_lv = [
        ...
        yi in zip(
            filter_data_product_level_total_toubu.done_lv_mean,
            filter_data_product_level_total_toubu.front_cate_one)]

    annotations_item_num = [
        dict(
            x=-0.1,
            y=yi,
            text=str(
    ...
    fig_kpi_toubu.update_layout(title=dict(text="头部等级KPI", font=dict(family='Microsoft YaHei', size=13),
                                ...
                                yaxis=dict(
        tickangle=0, tickfont=dict(
            family='Microsoft YaHei', size=10)),  # 设置轴标签旋转角度
    ...
    annotations_item_num_yao = [
        ...
        yi,
        zi in zip(
            filter_data_product_level_total_yaobu.done_lv_mean,
            filter_data_product_level_total_yaobu.front_cate_one,
            filter_data_product_level_total_yaobu.item_num_mean)]

    # 腰部进度条
    fig_kpi_yaobu = go.Figure(
        go.Bar(
            y=filter_data_product_level_total_yaobu.front_cate_one,
    ...
    fig_kpi_yaobu.update_layout(title=dict(text="腰部等级KPI", font=dict(family='Microsoft YaHei', size=13),
                                ...
                                yaxis=dict(
        tickangle=0, tickfont=dict(
            family='Microsoft YaHei', size=10)),
        showlegend=False,
        margin=dict(l=35, r=5, t=35, b=35),
        autosize=False
    )
    ...
    # 饼图数据联动-图形渲染
    data_yewuxian.log_date = pd.to_datetime(
        data_yewuxian.log_date, format='%Y-%m-%d')
    filter_data_yewuxian = data_yewuxian[(data_yewuxian.log_date >= start_date) & (
        data_yewuxian.log_date <= end_date)]

    filter_data_yewuxian_sum = filter_data_yewuxian.groupby(
        ['yewuxian', 'front_cate_one'], as_index=False).sum()
    filter_data_yewuxian_sum_total = filter_data_yewuxian_sum[
        filter_data_yewuxian_sum.yewuxian != 'total']

    print("业务线占比图-饼图")

    # 子图设置方式 注意doman是根据列和行来设置的，一行一个中括号
    fig_yewuxian = make_subplots(rows=2, cols=1, specs=[
                                 [{'type': 'domain'}], [{'type': 'domain'}]])

    fig_yewuxian.add_trace(
        go.Pie(
            labels=filter_data_yewuxian_sum_total.yewuxian,
    ...
    # 梯形图数据联动-图形渲染
    data_cate_zhanbi.log_date = pd.to_datetime(
        data_cate_zhanbi.log_date, format='%Y-%m-%d')
    filter_data_cate_zhanbi = data_cate_zhanbi[(data_cate_zhanbi.log_date >= start_date) &
                                               (data_cate_zhanbi.log_date <= end_date)]

    filter_data_cate_zhanbi_sum = filter_data_cate_zhanbi.groupby(
        ['front_cate_one'], as_index=False).sum()

    filter_data_cate_zhanbi_sum_df = filter_data_cate_zhanbi_sum[
        filter_data_cate_zhanbi_sum.front_cate_one != '全类目']
    ...
    filter_data_cate_zhanbi_sum_total_1 = filter_data_cate_zhanbi_sum.sort_values(
        by='total_zhanbi', ascending=False).head(11)
    ...
    fig_cate_zhanbi.add_trace(
        go.Funnel(
            name='UV-%',
            y=filter_data_cate_zhanbi_sum_total.front_cate_one,
    ...
    fig_cate_zhanbi.update_layout(title=dict(text='类目GMV&UV&在售商品数占比',
                                             font=dict(
                                                 family='Microsoft YaHei', size=13),
                                             xanchor='left', yanchor='top', xref='container', yref='container',
                                             x=0.25, y=0.98),
                                  # width=500,
                                  funnelmode='stack',
...
def update_graph_ca(selected_dropdown_value_ca, zhibiao_type_radio):  # 类目销售趋势回调数据
    ...
    for leimu in selected_dropdown_value_ca:
        zhibiao_download_df_gmv = data_category[data_category["category_one"] == leimu][[
            "log_date", "laiyuan", "category_one", "gmv"]]
        zhibiao_download_df_qty = data_category[data_category["category_one"] == leimu][[
            "log_date", "laiyuan", "category_one", "qty"]]
        zhibiao_download_df_cvr = data_category[data_category["category_one"] == leimu][[
            "log_date", "laiyuan", "category_one", "cvr"]]
        zhibiao_download_df_uv = data_category[data_category["category_one"] == leimu][[
            "log_date", "laiyuan", "category_one", "uv"]]
        zhibiao_download_df_kd_price = data_category[data_category["category_one"] == leimu][[
            "log_date", "laiyuan", "category_one", "kd_price"]]
        zhibiao_download_df_goods_nums = data_category[data_category["category_one"] == leimu][[
            "log_date", "laiyuan", "category_one", "goods_nums"]]

        zhibiao_download_df = {
            "销售额-$": zhibiao_download_df_gmv,
            "商详访客": zhibiao_download_df_uv,
            "转化率": zhibiao_download_df_cvr,
            "销量": zhibiao_download_df_qty,
            "客单价-$": zhibiao_download_df_kd_price,
        ...
        zhibiao_csv_string_download = zhibiao_data_download_df.to_csv(
            index=False, encoding='utf-8')
        zhibiao_csv_string_download = "data:text/csv;charset=utf-8," + \
            urllib.parse.quote(zhibiao_csv_string_download)
    ...
    chart_data_ca = {
        ...
        "转化率": traces_ca_cvr,
        "销量": traces_ca_qty,
        "客单价-$": traces_ca_kd_price,
        "在售商品数": traces_ca_goods_nums,
    }

    data_ca = [val_ca for sublist in chart_data_ca[zhibiao_type_radio]
               for val_ca in sublist]
...
def update_graph_level(selected_dropdown_value):
    ...
    for yewuxian in selected_dropdown_value:
        ...
        df_level_change = [
            val_level for sublist_level in traces_level_change for val_level in sublist_level]
        figure_levle_change = \
            {'data': df_level_change,
             'layout': go.Layout(colorway=['rgb(235, 12, 25)', 'rgb(234, 143, 116)',
                                 ...
                                 title=dict(
                                     text=f"{','.join(str(dropdown_level_change[j]) for j in selected_dropdown_value)} "
                                     f"等级变化趋势",
                                     font=dict(family='Microsoft YaHei', size=13), xanchor='left', yanchor='top',
                                     xref='container', yref='container', x=0.4, y=0.98),
                                 xaxis={'title': {'text': '日期 (可筛选)',
