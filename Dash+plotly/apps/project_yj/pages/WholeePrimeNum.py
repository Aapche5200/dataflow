...
sql_ms = ('''
select 日期,国家,会员数 from CFflows.UserPrime
/* where 日期 BETWEEN date_sub(curdate(),interval 30 day) and date_sub(curdate(),interval 1 day) */
order by 日期
''')

sql_PrimeALL = ('''
select 国家,sum(会员数) as "会员数" from CFflows.UserPrime
GROUP BY 国家
''')
...
annotations_uk = [dict(xref='paper', x=0.98,
                       # 取最后一行一列数值
                       y=(data_WholeePrime[data_WholeePrime["国家"]
                                           == "英国"]).values[-1][2],
                       xanchor='left',
                       yanchor='middle',
                       text="总数" + \
                       str(int(
                           data_PrimeAll[data_PrimeAll["国家"] == "英国"]["会员数"])),
                       font=dict(
...
annotations_gb = [dict(xref='paper', x=0.98,
...
annotations_all = [dict(xref='paper', x=0.98,
