...
where a.log_date between (GETDATE()-60) and (GETDATE()-1) 
ORDER BY a.log_date,a.front_cate_one,a.yewu_type
...
where a.log_date between (GETDATE()-30) and (GETDATE()-1) 
ORDER BY a.log_date,a.front_cate_one,a.yewu_type
''')
...
autofetch_front_cate_one_goods = ['总', 'Women\'s Shoes', 'Women\'s Clothing', 'Women\'s Bags', 'Watches',
                                  'Men\'s Shoes', 'Home',
...
layout = \
    html.Div([
        ...
        html.Div([
            html.Div([
                html.Div([
                    html.Div([
                        ...
                        html.Br(),
                        dcc.DatePickerRange(
                            id='date-picker-range-cate-yewuxian-ott',
                    ...
                    html.Div(
                        [
                            dcc.Dropdown(
                                id='front-cate-two-goods-ott',
                                # multi=True
                                style={'width': '150px', 'font-family': 'Microsoft YaHei',
                                       'font-size': 8, 'textAlign': 'left'}
                            )
                        ],
                        id='front-cate-two-container-goods-ott',
                        # className="padding-top-bot"
                    ),
                    html.Div([
            ...
            html.Div(
                buld_modal_info_overlay('DashCateSec', 'bottom', dedent("""
...
# 二级下拉 回调数据
@app.callback(Output('front-cate-two-container-goods-ott', 'children'),
              [Input('front-cate-one-goods-ott', 'value')]
              )
...
@app.callback([Output('my-graph-table-ca-ott', 'children'), Output('download-link-ott', 'href')],
              [Input('date-picker-range-cate-yewuxian-ott', 'start_date'),
               Input('date-picker-range-cate-yewuxian-ott', 'end_date'),
               Input('my-dropdown-table-ca-ott', 'value'),
               Input('front-cate-one-goods-ott', 'value'),
               Input('front-cate-two-goods-ott', 'value'),
               ], )
def update_table_ca(start_date, end_date,
    ...
    if front_cate_one_dash == '总' and front_cate_two_dash == '总':
        ...
        data_table_ca_past_total.rename(columns={'uv': 'uv_past', 'gmv': 'gmv_past', 'pv': 'pv_past',
                                                 'qty': 'qty_past', 'item_num': 'item_num_past',
        ...
        filter_data_table_ca_total = \
            filter_data_table_ca_total.sort_values(by='gmv', ascending=False).head(50)  # 降序

        filter_data_table_ca_total.rename(columns=
                                          {'yewu_type': '类型', 'front_cate_one': '类目', 'uv': '访客',
        ...
        data_table_ca_two = data_table_two[data_table_two['front_cate_one'] ==
                                           front_cate_one_dash]
        data_table_ca_two.log_date = pd.to_datetime(data_table_ca_two.log_date, format='%Y-%m-%d')
        data_table_ca_now = data_table_ca_two[
            (data_table_ca_two.log_date >= start_date) & (data_table_ca_two.log_date <= end_date)]
        data_table_ca_past = \
        ...
        data_table_ca_now_item_num_mean = \
            data_table_ca_now.groupby(by=
                                      ['front_cate_one', 'front_cate_two', 'yewu_type'], as_index=False).item_num.mean()
        data_table_ca_now_item_num_mean.rename(columns={'item_num': 'item_num_mean'}, inplace=True)
        data_table_ca_now_sum = \
            data_table_ca_now.groupby(by=
                                      ['front_cate_one', 'front_cate_two', 'yewu_type'], as_index=False).sum()
        data_table_ca_now_total = \
        ...
        filter_data_table_ca_total.rename(columns={'yewu_type': '类型',
                                                   'front_cate_one': '一级类目',
                                                   'front_cate_two': '二级类目',
                                                   'uv': '访客',
                                                   'gmv': '销售', 'qty': '销量',
        ...
        data_table_three.log_date = pd.to_datetime(data_table_three.log_date, format='%Y-%m-%d')
        data_table_ca_now = data_table_three[
            (data_table_three.log_date >= start_date) & (data_table_three.log_date <= end_date)]
        data_table_ca_past = \
        ...
        data_table_ca_now_total = \
            pd.merge(data_table_ca_now_sum, data_table_ca_now_item_num_mean,
                     on=['front_cate_one', 'front_cate_two', 'front_cate_three', 'yewu_type'])
        ...
        data_table_ca_past_item_num_mean = \
            data_table_ca_past.groupby(by=['front_cate_one', 'front_cate_two', 'front_cate_three', 'yewu_type'],
                                       as_index=False).item_num.mean()
        data_table_ca_past_item_num_mean.rename(columns={'item_num': 'item_num_mean'}, inplace=True)
        data_table_ca_past_sum = data_table_ca_past.groupby(
            by=['front_cate_one', 'front_cate_two', 'front_cate_three', 'yewu_type'], as_index=False).sum()
        data_table_ca_past_total = \
            pd.merge(data_table_ca_past_sum, data_table_ca_past_item_num_mean,
                     on=['front_cate_one', 'front_cate_two', 'front_cate_three', 'yewu_type'])
        ...
        data_table_ca_total = \
            pd.merge(data_table_ca_now_total, data_table_ca_past_total,
                     on=['front_cate_one', 'front_cate_two', 'front_cate_three', 'yewu_type'])
        print("表格now and past 数据合并")
        ...
        filter_data_table_ca_total = \
            data_table_ca_total[
                ['yewu_type', 'front_cate_one', 'front_cate_two', 'front_cate_three', 'uv', 'gmv', 'qty', 'cvr',
                 'kd_price', 'item_num_mean', 'pv_zb', 'gmv_zb', 'item_num_zb',
                 'uv_hb', 'gmv_hb', 'qty_hb', 'cvr_hb', 'kd_price_hb',
                 'item_num_hb']]
