...
con_mssql = pymssql.connect("172.16.92.2", "sa", "yssshushan2008", "CFcategory", charset="utf8")
...
from CFflows.dbo.CategoryPages
...
def update_table_ca(start_date, end_date, ):
    ...
    data_table_ca_now_sum = data_table_ca_now.groupby(by='front_cate_one', as_index=False).sum()

    data_table_ca_past_sum = data_table_ca_past.groupby(by='front_cate_one', as_index=False).sum()

    data_table_ca_past_sum.rename(columns={'HomeUv': 'HomeUv_past', 'CateUv': 'CateUv_past',
                                           'DetailUv': 'DetailUv_past', 'SearchUv': 'SearchUv_past',
                                           'HomeGmv': 'HomeGmv_past', 'CateGmv': 'CateGmv_past',
                                           'DetailGmv': 'DetailGmv_past', 'SearchGmv': 'SearchGmv_past',
                                           'HomeCvr': 'HomeCvr_past', 'CateCvr': 'CateCvr_past',
                                           'DetailCvr': 'DetailCvr_past', 'SearchCvr': 'SearchCvr_past', },
                                  inplace=True)
    ...
    filter_data_table_ca_total = \
        filter_data_table_ca_total.sort_values(by='HomeGmv', ascending=False).head(15)  # 降序
