...
where sol.is_delivery=0 and date(so.create_at) between date('2020-04-01') and date('2020-04-15') and date_id=date('2020-04-14') and tt.is_essential =1
group by date(so.create_at),case when a.write_uid=5 then 'seller' else 'cf' end
--order by date(so.create_at)
''')

data_OrderNum = pd.read_sql(sql_hive, con_hive)
