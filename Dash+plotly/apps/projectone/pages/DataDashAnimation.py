...
AVG(在售商品数) as 均在售商品数,sum([支付商品件数]) as 销量, sum(独立访客) as 商详访客 from CFcategory.dbo.category_123_day
WHERE (二级类目 = ' ' or 二级类目 is null ) and 一级类目 !='全类目' and 日期 BETWEEN '2019-01-01' and '2019-12-31'
GROUP BY month(日期),业务类型,一级类目 
HAVING sum(支付金额)  >=0
Union All
SELECT (str(month(日期))+12) as yearmonth,业务类型,一级类目 as category_one,sum(支付金额) as 支付金额,
AVG(在售商品数) as 均在售商品数,sum(支付商品件数) as 销量, sum(独立访客) as 商详访客 from CFcategory.dbo.category_123_day
WHERE (二级类目 = ' ' or 二级类目 is null ) and 一级类目 !='全类目' and 日期 BETWEEN '2020-01-01' and '2020-12-31'
GROUP BY (str(month(日期))+12),业务类型,一级类目 
HAVING sum(支付金额)  >=0
ORDER BY yearmonth,业务类型,一级类目
''')
